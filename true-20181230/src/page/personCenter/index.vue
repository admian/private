  ...
  <div id="personCenter">
    ...
    <x-header :left-options="{backText:''}" v-if="useList.id!=my_userId">
      ...
      <div class="nick ">
        ...
        <img class="sex_ico fl" :class="useList.sex==0 ?'boy' : 'girl'" :src="useList.sex==0 ? require('../../assets/img/self/boy_sex.png') :require('../../assets/img/self/girl_sex.png')"/>
        </div>
       ...
       <!-- <scroller v-model="scrollerStatus" height="-46" lock-x scrollbar-y ref="scroller" :bounce="true"
          ...
          <div class="container" v-show="key==1">
            <div class="no-connect" v-if="!allProdution.length">
              <div v-if="useList.id==my_userId">
                ...
                <div class="btn-work" @click="publishWork">创作首个作品</div>
              </div>
              <div v-if="useList.id!=my_userId">
          ...
          <div class="container" v-show="key==3">
            <div class="no-connect" v-if="!allFavorite.length">
              ...
              <div class="btn-work" @click="chooseIndex">再去瞧瞧</div>
            </div><!--收藏为空时-->

            <div class="sc-wrap">
              <div v-for="(item,index) in allFavorite" :key="index">
                <router-link :to="'/prodution?id='+item.id" class="item" :class="item.videoType==1 ? 'lg':''">
                  <div class="act-like">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      publishWork(){
        window.location.href = '/publishWork';
      },
      chooseIndex(){
        window.location.href = '/chooseIndex';
      },
      clickSet(){
        ...
        if(this.userId!=this.my_userId) {
          this.$post('api/offline/user/queryAllFavoriteByUserIdByPage', {pageSize:limit,pageNum:this.page3,userId:this.userId}).then(
            res => {
              ...
              if (res.list && res.pages <= this.page3) {
                this.addTemp = false;
              }
            });
        }else{
...
<style scoped>
  ...
  .waterfall .item .pic {width: 100%;border-radius: 5px;}

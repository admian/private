<template>
    <div id='editData'>
       ...
       <div class="userPhoto" @click="changePic">
         <p align="center"><img :src="userList.headUrl ? baseUrl+userList.headUrl: './static/image/iy_pic.png' " width="1.26rem" height="1.26rem"></p>
      ...
      <div class="container">
          <div class="message">
            <ul>
              ...
              <li><span>生日</span><p class="bird">{{birdTime}}</p><p class="fr" @click="birdState"><img src="../../assets/img/self/back.png" class="backicon"></p></li>
      ...
      <popup v-model="birdShow">
        <datetime-view v-model="birdTime" ref="datetime" :min-year=1790 :max-year=2018></datetime-view>
      </popup>
...
<script>
    ...
    export default {
        ...
        methods: {
           ...
           //更换头像
            changePic(){
              window.location.href = '/editPhone';
            },
            //选择性别
            addSex(i){
              if(i==1){
            ...
            saveInfo(){
              console.log('保存');
              //console.log(this.birdTime);
              let dateTime = new Date(this.birdTime);
              let birthday = dateTime.getTime();
              ...
              //更新后的演艺
              let objlist=[];
              this.userLabel.forEach(function(i){
                let objs={
                  labelsId:i.labelsId
                }
                objlist.push(objs);
        ...
        created(){
          ...
          this.$post('api/user/getUserInfo').then(res=>{
            this.userList=res;
            //演艺标签
            this.userLabel=res.userLabelsList;
            this.userLabel.forEach(function(i){
            ...
            //生日
            let date = new Date(this.userList.birthday);//时间戳为10位需*1000，时间戳为13位的话不需乘1000

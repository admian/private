...
<!--      <scroller v-show="isShow" v-model="scrollerStatus" height="-46" lock-x scrollbar-y ref="scroller" :bounce="true"
        ...
        <div class="waterfall">
          <div :class=" i.videoType == 1 ? 'item lg' : 'item'" v-for="i,index in list" :key="index">
            <router-link :to="'/activityDetails?type='+i.channelId+'&&id='+i.id+'&&isImpromptu=impromptu'">
              <img class="pic" :src="i.productionCover">
            </router-link>
            <router-link :to="'/person?id='+i.userId" class="user-info clearfix">
              <img class="fl" :src="baseUrl + i.headUrl"/>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getData(isEmpty){
        let limit = 10;
        this.$post('/api/offline/banner/queryAllBannerByChannelId/'+5, {channelId: 3,pageSize:limit,pageNum:this.page}).then(
          res => {
            this.isLoading = false;
            if (isEmpty) {
              this.list = res.list;

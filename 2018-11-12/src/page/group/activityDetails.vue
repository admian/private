<template>
  <div id="activityDetails">
    <x-header :left-options="{backText: title}"><a slot="right" @click="SETSTATE({type:'showShare',val:true})"><img
      ...
      <div class="actUser clearfix" v-if="activeInfo">
        <div class="fl" @click="isAttention(activeInfo.userId,activeInfo.isAttention)">
          <div class="userPhoto fl">
            ...
            <div class="focus"><img v-if="!activeInfo.isAttention" src="../../assets/img/dream/关注@2x.png"><!--<Icon type="ios-add"/>--></div>
          </div>
        ...
        <div class="llTwo fr">
          <div class="fl" @click="upActNum" :class="activeInfo.isFavorite ? 'like' : 'like_sel'"><i class="num">{{activeInfo.favoriteCount}}</i>
          </div>
          <div class="leave fl" @click="showPopup"><i class="num">{{activeInfo.pushCount}}</i></div>
        </div>
      </div>
      <!------------------------------------------------------------------------->
        ...
        <div class="slider-wrap">
          <ul class="list-nav clearfix">
            <li class="list-item fl" v-for="i,index in list" :key="index" @click="changeVideo(index)">
              ...
              <router-link :to="'/person?id='+i.userId" class="recom clearfix">
                <div class="fl recoml">
        ...
        <scroller v-show="isShow" v-model="scrollerStatus" height="7rem" lock-x scrollbar-y ref="scroller"
          ...
          <ul>
            <li v-for="i,index in commentList" :key="index" class="clearfix msg_item">
              <router-link :to="'/person?id='+i.userId"><img :src="baseUrl+ i.headUrl" class="fl user_pic"/></router-link>
              <div class="fl cont">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getVideoInfo() {
        this.$post('/api/offline/entertainment/getProductionInfoById/' + this.list[this.listIndex].id, {}).then(res => {
          this.activeInfo = res;
      ...
      /*评论点赞*/
      upCommentNum(i) {
        let url = '/api/entertainment/updateProductionEvaluatePraise';
        if (this.routerType == 'impromptu') {
    ...
    created() {
      ...
      if (this.type) {
        ...
        this.$post('/api/offline/entertainment/getProductionInfoById/' + this.id, {}).then(res => {
          this.activeInfo = res;
      ...
      if (this.is_login) {
        this.$post('/dream/getPushSum', {}).then(res => {

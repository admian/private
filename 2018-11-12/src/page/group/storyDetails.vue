<template>
  <div id='storyDetails'>
    <x-header :left-options="{backText: ''}"><a slot="right" @click="SETSTATE({type:'showShare',val:true})"><img
      class="share_ico" src="../../assets/img/share_ico.png"></a></x-header>
    <div class="start-info">
      <p class="story-title">{{storeInfo.storyName}}</p>
      <div class="user-info clearfix">
        <router-link :to="'/person?id='+ storeInfo.userId" class="link fl clearfix">
          <img :src="baseUrl + storeInfo.headUrl" class="user-p"/>
          <div class="user-n">
            <p>{{filtName(storeInfo.nickName)}}</p>
            <p class="label-n">{{storeInfo.storyTitleName}}</p>
          </div>
        </router-link>
        <div class="focus" :class="storeInfo.isAttention ?'focus-yes': 'focus-no'"
             @click="clickFocus(storeInfo.userId,storeInfo.isAttention)">{{storeInfo.isAttention ? '已关注' : '关注'}}
        </div>
      </div>
      <div v-if="storeInfo.voiceList.length" class="video-info" @click="palyAudio">
      ...
      <div class="list-pic" v-if="storeInfo.pictureList.length">
        <img v-for="i,index in picList" :key="index" :src="i.sourceUrl" v-if="showPic(index)">
      </div>
      <div class="loadMore" @click="showMore = !showMore" v-if="storeInfo.pictureList.length > 3">
    ...
    <div class="reply-wrap">
      ...
      <div class="comment_item" v-for="i,index in commentList" :key="index">
        <div class="comment_item_bd">
          <div class="clearfix">
            <router-link :to="'/person?id='+i.userId" class="fl link">
              <img :src="baseUrl + i.headUrl" class="fl nick-p">
              <div class="nickname">
        ...
        <div class="reply clearfix" v-if="i.storyEvaluateReplies">
          <ul>
            <li v-for="k,indexK in i.storyEvaluateReplies" :key="indexK">
              <router-link class="link" :to="'/person?id='+k.userId">{{filtName(k.nickName)}}</router-link>
              ：<span class="com_txt">{{k.replyBody}}</span></li>
          </ul>
          <Icon v-if="i.replyCount > 1" class="fr ico" size="24" type="ios-arrow-down"
                @click="getReply(i.id,index)"/>
        </div>
    ...
    <div class="comment_info">
      ...
      <img v-if="!txtarea"
           :src="isStory ? require('../../assets/img/like_sel_ico.png'):require('../../assets/img/self/like_story.png')"
           class="like_story fr" @click="likeStory">
    </div>
    <!--评论区结束-->
    <audio style="display: none" controls :src="storeInfo.voiceList[0].sourceUrl"
           ref="audio"
           @play="ready"
...
<script>
  import {XHeader, cookie, Popup, Scroller} from 'vux'
  ...
  export default {
    ...
    methods: {
      ...
      likeStory() {
        this.$post('/api/story/updateStoryFavorite', {storyId: this.storeInfo.id}).then(res => {

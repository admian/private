  ...
  <div id="personCenter">
    <x-header :left-options="{backText:''}"><a slot="right" @click="SETSTATE({type:'showShare',val:true})"><img
      ...
      <div class="user clearfix">
        ...
        <div class="focus" :class="isFocus ?'focus-yes': 'focus-no'" @click="clickFocus" >{{focusTxt}}</div><!--v-if=""-->
      </div>
      <div class="nick clearfix">
        <span class="title fl">{{nameTxt(useList.nickName)}}</span>
      ...
      <div class="numbers clearfix">
        ...
        <i class="num fl">{{mberList.attentionCount}}</i>
        <span class="fl">关注</span>

        <i class="fl ico"> / </i>

        <i class="num fl">{{mberList.fansCount}}</i>
        <span class="fl">粉丝</span>

        <i class="fl ico"> / </i>

        <i class="num fl">{{mberList.favoriteCount}}</i>
        <span class="fl">获赞</span>

        <i class="fl ico"> / </i>
        <router-link to="/mystory">
          <i class="num fl">{{mberList.storyCount}}</i>
          <span class="fl">故事</span>
        </router-link>
      </div>
      <Collapse simple class="sel_tab" ><!--v-if=""-->
        <Panel name="1">
          艺术取向
    ...
    <div class="works">
      <tab class="tabs" :line-width="2" custom-bar-width=".25rem">
        ...
        <tab-item :selected="num===3" @on-item-click="onItemClick(3)" >10 收藏</tab-item><!--v-if-->
      </tab>
      <div>
